...
const InsertFish: React.FC = () => {
  const [modal, setModal] = useState<boolean>(false);
  const [modalMessage, setModalMessage] = useState<string>('');
  const [formData, setFormData] = useState<FormData>({
    name: '',
    categoryName: fishCategories[0], // Default to the first category
    image: '',
    description: '',
    price: '',
    quantity: '',
  ...
  // Handle category selection (like ShopPage)
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      setModalMessage('Fish inserted successfully!');
    } catch (error) {
      if (axios.isAxiosError(error) && error.response) {
        console.error('Insert error:', error.response.data);
        setModalMessage(error.response.data.message || 'Failed to insert fish. Please try again.');
      } else {
        console.error('Insert error:', error);
        setModalMessage('Failed to insert fish. Please try again.');
      }
    } finally {
      setModal(true);
      setLoading(false);
            ...
            <Card className="shadow-lg">
              <Form onSubmit={handleSubmit} className="form">
                <CardHeader className="text-center" style={{ backgroundColor: 'rgb(55, 162, 245)', color: '#fff' }}>
                  <CardTitle className="title-up" tag="h3">Add New Fish</CardTitle>
                </CardHeader>
                <CardBody>
                  {errorMessage && (
                  ...
                  {/* Category Selection (Similar to ShopPage) */}
                  <Row className="mb-3">
                    <Col xs="4">
